...
<head>
	...
	<title>Informations importantes sur les médicmaents</title>
</head>
<body>
	<jsp:include page="nav.jsp"></jsp:include>
	<br>
 	
	<form action="InformationsImportantes" method="get">
	  <div class="row">
	    <div class="col col-4">
 	...
 	<%if (session.getAttribute("informations") != null && session.getAttribute("informationsVide")==null){ %>
   		<div class="accordion" id="accordionExample">
   		
  	  <%for (DTO_CIS_bdpm e : (ArrayList<DTO_CIS_bdpm>)session.getAttribute("informations")){ %>		
		    <div class="card">
			    ...
			    <div id=<%=e.getAttributes().get("forme") %> class="collapse" aria-labelledby=<%=e.getAttributes().get("cis") %> data-parent="#accordionExample">
			      <div class="card-body">
			  	      ...
			  	      <form class="form-inline" action="InformationsImportantes" method="post">
						  <div class="form-group mb-2">
						    <input type="text" readonly class="form-control plaintext" name="cis" value=<%=e.getAttributes().get("cis") %> hidden>
						  </div>
						  <button type="submit" class="btn btn-outline-dark">Voir les infos importantes</button>
						</form>
			      </div>
			    </div>
			  </div>
  	...
  	<%}else if(session.getAttribute("informationsVide")==null){ %>

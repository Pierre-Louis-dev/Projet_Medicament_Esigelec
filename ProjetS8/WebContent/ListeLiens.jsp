...
<head>
	...
	<title>Liste des liens de la transparence pour les SMR et ASMR</title>
</head>
<body>
	<jsp:include page="nav.jsp"></jsp:include>
	<br>
 	
	<form action="LiensPage" method="post" class="aligne">
	  <div class="row">
	    <div class="col col-4">
	      <input type="text" class="form-control" placeholder="Nom du médicament" name="med">	      
	    </div>
	    <div class="col col-3">
	 ...
	 <%if (session.getAttribute("lienHAS")!=null && session.getAttribute("fromSMR")!=null){ %>
		  <table class="table table-striped">
			  <thead>
			    ...
			    <tbody>
				<tr>
				<% DTO_HAS_LiensPageCT_bdpm e = (DTO_HAS_LiensPageCT_bdpm) session.getAttribute("lienHAS"); %>
			      <th scope="row"><%=e.getAttributes().get("codeDossier") %></th>
			      <td><a href=<%=e.getAttributes().get("lien") %>><%=e.getAttributes().get("lien")%></a></td>
			    </tr>
			  </tbody>
			</table>
			<%}else if (session.getAttribute("lienMed")!=null){%>
				<div class="accordion" id="accordionExample">
			   		
			  	  <%for (DTO_CIS_bdpm e : (ArrayList<DTO_CIS_bdpm>)session.getAttribute("lienMed")){ %>		
					    <div class="card">
						    ...
						    <div id=<%=e.getAttributes().get("forme") %> class="collapse" aria-labelledby=<%=e.getAttributes().get("cis") %> data-parent="#accordionExample">
						      <div class="card-body">
						  	      CIS : <%=e.getAttributes().get("cis") %><br>
						  	      Titulaires : <%=e.getAttributes().get("titulaires") %><br>
						  	      Surveillance : <%=e.getAttributes().get("surveillance") %><br>
								  <form class="form-inline" action="LiensPage" method="get">
								  	<div class="form-group mb-2">
								    	<input type="text" readonly class="form-control plaintext" name="cis" value=<%=e.getAttributes().get("cis") %> hidden>
								  	</div>
								  	<button type="submit" class="btn btn-outline-dark">Voir le lien</button>
								  </form>
						      </div>
						    </div>
						  </div>

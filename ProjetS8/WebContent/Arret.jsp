...
<head>
	...
	<title>Medicaments non commercialisés</title>
</head>
<body>
	...
	<form action="Recherche" method="POST">
 	...
 	<%if (session.getAttribute("arretes") != null && session.getAttribute("arretesVide")==null){ %>
   		<div class="accordion" id="accordionExample">
   		
  	  <%for (DTO_CIS_bdpm e : (ArrayList<DTO_CIS_bdpm>)session.getAttribute("arretes")){ %>		
		    <div class="card">
			    ...
			    <div id=<%=e.getAttributes().get("forme") %> class="collapse" aria-labelledby=<%=e.getAttributes().get("cis") %> data-parent="#accordionExample">
			      <div class="card-body">
			  	      Forme pharmaceutique : <%=e.getAttributes().get("forme") %><br>
			  	      Voies d'administration : <%=e.getAttributes().get("voies") %><br>
			  	      ...
			  	      <div class="form-inline">
				  	      <form class="form-inline" action="Test" method="get">
							  <div class="form-group mb-2">
							    <input type="text" readonly class="form-control plaintext" name="cis" value=<%=e.getAttributes().get("cis") %> hidden>
							  </div>
							  <button type="submit" class="btn btn-outline-dark">Voir les présentations disponibles</button>
							</form>
							<%if(session.getAttribute("actif")!=null && e.getAttributes().get("generique")!= null){ %>
							<br>
							<form class="form-inline" action="Test" method="post">
							  <div class="form-group mb-2">
							    <input type="text" readonly class="form-control plaintext" name="cis" value=<%=e.getAttributes().get("cis") %> hidden>
							  </div>
							  <button type="submit" class="btn btn-outline-dark">Voir les groupes génériques associés</button>
							</form>	 
							<%} %>
						</div>
			      </div>
  	...
  	<%}else if(session.getAttribute("arretesVide")==null){ %>

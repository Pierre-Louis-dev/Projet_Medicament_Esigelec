...
<head>
	...
	<title>Compositions des médicaments</title>
</head>
<body>
	<jsp:include page="nav.jsp"></jsp:include>
	<br>
 	
	<form action="CompositionController" method="POST">
	  <div class="row">
	    <div class="col col-4">
 	...
 	<%if (session.getAttribute("compositions") != null && session.getAttribute("compositionsVide")==null){ %>
   		<div class="accordion" id="accordionExample">
   		
  	  <%for (DTO_CIS_bdpm e : (ArrayList<DTO_CIS_bdpm>)session.getAttribute("compositions")){ %>		
		    <div class="card">
			    ...
			    <div id=<%=e.getAttributes().get("forme") %> class="collapse" aria-labelledby=<%=e.getAttributes().get("cis") %> data-parent="#accordionExample">
			      <div class="card-body">
			  	      Etat : <%=e.getAttributes().get("forme") %><br>
			  	      Date d'autorisation de mise sur le marché : <%=e.getAttributes().get("dateAam") %><br>
			  	      Titulaires : <%=e.getAttributes().get("titulaires") %><br>
			  	      
			  	      <form class="form-inline" action="CompositionController" method="get">
						  <div class="form-group mb-2">
						    <input type="text" readonly class="form-control plaintext" name="cis" value=<%=e.getAttributes().get("cis") %> hidden>
						  </div>
						  <div class="form-check form-check-inline">
							  <input class="form-check-input" type="checkbox" id="checkSA" name="substanceActive">
							  <label class="form-check-label" for="checkSA">Substances actives</label>
							</div>
							<div class="form-check form-check-inline">
							  <input class="form-check-input" type="checkbox" id="checkFT" name="fractionTherapeutique">
							  <label class="form-check-label" for="checkFT">Fractions thérapeutiques</label>
							</div>
						  <button type="submit" class="btn btn-outline-dark">Voir la composition</button>
						</form>
			      </div>
			    </div>
			  </div>
  	...
  	<%}else if(session.getAttribute("compositionsVide")==null){ %>

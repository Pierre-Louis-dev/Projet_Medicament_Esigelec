...
/**
 * JFrame pour afficher l'historique de recherche des utilisateurs
 * @author Groupe 4
 *
 */
public class HistoriqueUtilisateurs extends JFrame implements ActionListener{
	...
	public HistoriqueUtilisateurs() {
		setTitle("Historique des recherches des utilisateurs");
		setSize(610, 600);
		setResizable(false);
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		getContentPane().setLayout(null);
		
		HistoriqueRecherche enr = new HistoriqueRecherche();
		ArrayList<String> historique = enr.getHistorique();
		String[] header= {"Date","Login","Action"};
		String[][] donnees = new String[historique.size()][4] ;
		for(int i=0;i<historique.size();i++) {
			String[] parts = historique.get(i).split("\t",3);			
			donnees[i][0]=parts[0];
			donnees[i][1]=parts[1];
			donnees[i][2]=parts[2];
		}
